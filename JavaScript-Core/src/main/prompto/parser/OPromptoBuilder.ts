...
import {Interval, ParserRuleContext, Token, TokenStream} from 'antlr4';
import OParserListener from './OParserListener';
import OCleverParser from './OCleverParser';
import {Dialect, OParser, OLexer, Section} from "./index";
import {
    Annotation,
    Argument,
...
import {
    ...
    IfElementList,
    IfStatement,
    IStatement, RaiseStatement, ReadStatement,
    RemoteCall,
    ReturnStatement,
    StatementList,
    SwitchCase,
...
import {
    ...
    An_expressionContext,
    IsAnExpressionContext,
    EmptyCategoryMethodListContext,
    CurlyCategoryMethodListContext, SingleStatementContext, CurlyStatementListContext, Filtered_list_expressionContext
} from "./OParser";
import {
    AndExpression,
    ArrowExpression,
...
export default class OPromptoBuilder extends OParserListener {
    ...
    constructor(parser: OCleverParser) {
        super();
    ...
    exitMethod_call_statement = (ctx: Method_call_statementContext) => {
        ...
        const call = this.getNodeValue<UnresolvedCall>(ctx._method);
        if(call)
            call.setParent(parent!);
        const name = this.getNodeValue<Identifier>(ctx._name);
        const stmts = this.getNodeValue<StatementList>(ctx._stmts);
        if (call && (name || stmts))
            this.setNodeValue(ctx, new RemoteCall(call?.caller, call.args!, new ThenWith(name!, stmts!)));
        else
    ...
    exitMethodSelector = (ctx: MethodSelectorContext) => {
        const call = this.getNodeValue<UnresolvedCall>(ctx._method)!;
        if (call.caller instanceof UnresolvedIdentifier) {
            const caller = new UnresolvedSelector(null, call.caller.id);
            caller.copySectionFrom(call.caller);
            call.caller = caller;
        }
        this.setNodeValue(ctx, call);
    }
    ...
    exitType_expression = (ctx: Type_expressionContext) => {
        const name = this.getNodeValue<Identifier>(ctx._name);
        this.setNodeValue(ctx, new TypeExpression(new CategoryType(name!)))
    }
    ...
    exitBlob_expression = (ctx: Blob_expressionContext) => {
        this.setNodeValue(ctx, new BlobExpression(this.getNodeValue<IExpression>(ctx.expression())!));
    }
    ...
    exitArrow_prefix = (ctx: Arrow_prefixContext) => {
        ...
        const argsSuite = this.getHiddenTokensBefore(ctx.EGT().symbol);
        const arrowSuite = this.getHiddenTokensAfter(ctx.EGT().symbol);
    ...
    exitAddExpression = (ctx: AddExpressionContext) => {
        ...
        const exp = ctx._op.type == OParser.PLUS ? new PlusExpression(left!, right!) : new SubtractExpression(left!, right!);
        this.setNodeValue(ctx, exp);
    }
    ...
    exitConstructorFrom = (ctx: ConstructorFromContext) => {
        ...
        const args = this.getNodeValue<ArgumentList>(ctx._args) || null;
    ...
    exitJsx_attribute = (ctx: Jsx_attributeContext) => {
        ...
        const suite = value ? this.getHiddenTokensAfter(ctx._value.stop!) : null;
    ...
    exitJsx_fragment = (ctx: Jsx_fragmentContext) => {
        const openingSuite = this.getHiddenTokensAfter(ctx.jsx_fragment_start().stop!);
    ...
    exitJsx_opening = (ctx: Jsx_openingContext) => {
        const name = this.getNodeValue<Identifier>(ctx._name);
        const nameSuite = this.getHiddenTokensAfter(ctx._name.stop!);
    ...
    exitDerivedList = (ctx: DerivedListContext) => {
        ...
        this.setNodeValue(ctx, new IdentifierList(undefined, item));
    }
    ...
    exitAn_expression = (ctx: An_expressionContext) => {
        this.setNodeValue(ctx, this.getNodeValue(ctx._typ));
    }

    exitIsAnExpression = (ctx: IsAnExpressionContext) => {
        const left = this.getNodeValue<IExpression>(ctx._left);
        const type = this.getNodeValue<IType>(ctx._right);
        const right = new TypeExpression(type!);
        const op = ctx.NOT() ? EqOp.IS_NOT_A : EqOp.IS_A;
        this.setNodeValue(ctx, new EqualsExpression(left!, op, right))
    }


    exitEmptyCategoryMethodList = (ctx: EmptyCategoryMethodListContext) => {
        this.setNodeValue(ctx, new MethodDeclarationList);
    }
    ...
    exitSingleStatement = (ctx: SingleStatementContext) => {
        const stmt = this.getNodeValue<IStatement>(ctx._stmt);
        this.setNodeValue(ctx, new StatementList(undefined, stmt!));
    }
    ...
    exitFiltered_list_expression = (ctx: Filtered_list_expressionContext) => {
        ...
        this.setNodeValue(ctx, new FilteredExpression(source, exp));
    }
