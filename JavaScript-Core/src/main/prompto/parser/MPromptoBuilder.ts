...
import {Dialect, EParser, MParser, Section} from "./index";
import {
    Annotation,
    Argument,
...
import {
    ...
    Copy_fromContext, MethodExpressionContext, Type_expressionContext, TypeExpressionContext
} from "./MParser";
import {
    AndExpression,
    ArrowExpression,
...
export default class MPromptoBuilder extends MParserListener {
    ...
    isNotIndent(tree: antlr4.tree.ParseTree): boolean {
        if(tree instanceof antlr4.tree.TerminalNode)
            return tree.symbol.type != MParser.INDENT;
        else
            return false;
    }
    ...
    exitIndex_clause = (ctx: Index_clauseContext) => {
        const indices = ctx._indices == null ? new IdentifierList() : this.getNodeValue<IdentifierList>(ctx._indices);
        this.setNodeValue(ctx, indices);
    }



    exitDerived_list = (ctx: Derived_listContext) => {
    ...
    exitAttribute_declaration = (ctx: Attribute_declarationContext) => {
        ...
        const indices = ctx.index_clause() ? this.getNodeValue<IdentifierList>(ctx.index_clause()) : null;
