...
function ETestParser(code, addLF) {
    presto.parser.ECleverParser.call(this, code);
    var lexer = this.getTokenStream().tokenSource;
    lexer.addLF = addLF;
    return this;
}

ETestParser.prototype = Object.create(presto.parser.ECleverParser.prototype);
ETestParser.prototype.constructor = ETestParser;

ETestParser.prototype.value = function (tree) {
    var builder = new presto.parser.EPrestoBuilder(this);
...
exports.testLiteral = function (test) {
    ...
    test.ok(sl instanceof presto.literal.IntegerLiteral);
    test.done();
};
...
exports.testEmptyTuple = function (test) {
    ...
    test.ok(tl instanceof presto.literal.TupleLiteral);
    test.done();
};
...
exports.testMemberExpression = function (test) {
    ...
    test.ok(me instanceof presto.expression.MemberSelector);
    test.equal("name", me.name);
    test.ok(me.parent instanceof presto.grammar.UnresolvedIdentifier);
...
exports.testMethod1Parameter1Statement = function (test) {
    ...
    test.ok(ad.args);
    var expected = new presto.grammar.CategoryArgument(new presto.type.CategoryType("Person"), "p", null);
    test.ok(presto.utils.arrayContains(ad.args, expected));
...
exports.testMethod1Extended1Statement = function (test) {
    ...
    test.equal("printName", ad.name);
    test.ok(ad.args);
    var expected = new presto.grammar.CategoryArgument(new presto.type.CategoryType("Object"), "o", new presto.grammar.IdentifierList("name"));
    test.ok(presto.utils.arrayContains(ad.args, expected));
    test.ok(ad.statements);
    writer = new presto.utils.CodeWriter(presto.parser.Dialect.E)
    ad.statements[0].toDialect(writer);
...
exports.testMethod1Array1Statement = function (test) {
    ...
    test.equal("printName", ad.name);
    test.ok(ad.args);
    var expected = new presto.grammar.CategoryArgument(
        new presto.type.ListType(
            new presto.type.CategoryType("Option")), "options", null);
    test.ok(presto.utils.arrayContains(ad.args, expected));
    test.ok(ad.statements);
    writer = new presto.utils.CodeWriter(presto.parser.Dialect.E)
    ad.statements[0].toDialect(writer);
...
exports.testSimpleListLiteral = function (test) {
    ...
    test.ok(literal.expressions[0] instanceof presto.grammar.UnresolvedIdentifier);
